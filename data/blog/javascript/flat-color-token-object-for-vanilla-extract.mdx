...
`tailwindCSS의 경우`도 tailwind.config.js를 통해 비슷한 기능을 제공하지만, 생성된 util class에 대해
타입추론이 되지 않기 때문에 `추후 토큰의 이름이 변경되면 존재하지 않는 util class가 감지되지 않는 단점`이 있어서 결과적으로 vanilla-extract를 선택하게 되었다(다른 이유도 있지만 이건 나중에 소개하는걸로..)
