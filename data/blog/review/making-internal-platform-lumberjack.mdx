...
변경 가능성과 확장 가능성에 초점을 두고 기술 스택을 선택했습니다.
...
ChatGPT Desktop App이나 Postman, Notion 등의 Native App들도 내부적으로 chat log, system log들을 이용한 기능을 만들기 위해 Native App으로 제작합니다.(물론 WebView를 적극적으로 활용합니다)

하지만 럼버잭은 조금 다릅니다. File System 외에도 Core Service 바이너리 실행, Folder Permission 허용 기능을 포함해야하고 또한 외부 RestAPI 서비스를 호출해야합니다.

따라서 Electron App의 IPC(Internal Procedure Call)을 이용하여 Node 기능을 사용하기 보다는, Rest API를 채택하여 Frontend와 Backend 개발 프로세스를 명시적으로 구분하는게 더 낫다고 판단했습니다.
...
하지만 github에서 다른 개발자가 올려놓은 Boilerplate를 참고했을 때 다음과 같은 아쉬운 점이 있었습니다

* Frontend와 Backend를 Monorepo로 분리한 Reference가 없다
	* 다만 Backend App Build
* 대부분의 Backend Service로직을 Cloud에 배포된 환경에 의존하고, File System 조작에 대해서 IPC를 이용해서 기능을 구현한다
* 
...
사실 Fullstack을 할 수는 있었지만, 개인적으로 Frontend가 재밌고 흥미로운 것들이 많아서 Frontend 개발을 하고 있었습니다...만

제 개인적인 아젠다가 `조직에 도움이 되면 무엇이든 하겠다. 문제 해결할 수만 있다면 된다`입니다.

그래서 지금 회사에서 `개발 시간을 최적화하기 위해 Platform을 제작한다`는 기회가 왔을때, 프로젝트 리드로써 임할 수 있었습니다.

하지만 개발 난이도에 대해서 오만하게 생각했던 것 같습니다.

왜냐하면 `개발 프로세스를 빠르게 만들어 성공하는 제품을 만든다는 목표`와 함께 `기존 서비스에 있던 문제점들(자동화의 부제, Runtime에서 테스트가 필요한 부분)을 반복하지 않는다`도 있었기 때문입니다.

추가로 Frontend, Backend 로직을 넘어서 Desktop Native Build, Binary(Redis, PostgreSQL, DynamoDB) Build, Codesign 등을 해야했거든요.
