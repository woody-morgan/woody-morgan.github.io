---
title: 'Next.js 어플리케이션 최적화하기(feat. Vercel)'
date: '2022-09-30'
tags: ['nextjs']
draft: false
...
게다가 이 태그는 Netlify, Vercel, Cloudinary 등의 이미지 호스팅 서비스(CDN)와 연동하여 사용할 수 있다. next.config.js에 간단히 설정만 하면 전세계에 CDN을 통해
...
![Lighthouse SEO 점수](https://d.pr/i/I7vgtW.png)
...
이렇게 배포된 Serverless Function은 Vercel에서 제공하는 CDN을 통해 호출되는데, 이 CDN이 기본적으로 San Francisco에 위치하고 있다.

즉 한국에서 한 사용자가 서비스 페이지를 요청하면(Serverside에서 호출되는 함수가 있고, 서비스 서버가 한국에 있다고 가정)

1. Vercel 미국 CDN에서 한국으로 api 전송 및 응답 (미국 -> 한국 -> 미국)
...
정말 많은 Network탭 분석을 했는데, 결국 Vercel에서 제공하는 CDN을 한국으로 변경하면 해결할 수 있었다.

변경하는 방법은 간단하다. Vercel에 배포된 프로젝트의 Settings에서 CDN Location을 변경하면 된다.
