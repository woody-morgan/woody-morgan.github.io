...
해당 글도 `Javascript` 기저에 깔린 `본질`에 대해 의문을 추구하다가, 우연히 [자바스크립트는 왜 프로토타입을 선택했을까](https://medium.com/@limsungmook/%EC%9E%90%EB%B0%94%EC%8A%A4%ED%81%AC%EB%A6%BD%ED%8A%B8%EB%8A%94-%EC%99%9C-%ED%94%84%EB%A1%9C%ED%86%A0%ED%83%80%EC%9E%85%EC%9D%84-%EC%84%A0%ED%83%9D%ED%96%88%EC%9D%84%EA%B9%8C-997f985adb42) 라는 글을 발견해서 썼습니다.
...
밑에는 이 글을 쓰면서 생각한 프로그래밍의 본질에 대해 적었습니다.
...
대부분 아는 `해리포터 시리즈, 반지의 제왕 시리즈` 소설책들을 예를 들어봅시다

두 책 모두 훌륭하지만,  그 안에서 쳅터 구성, 문맥 배치, 어체 등은 아예 다릅니다. 하지만 많은 사람들이 둘 다 훌륭한 책이라고 느낍니다
